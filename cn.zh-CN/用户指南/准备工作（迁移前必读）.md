# 准备工作（迁移前必读） {#concept_778499 .concept}
...
2.  确保账号余额大于等于100元（充值或代金券皆可）。
    ...
    **说明：** SMC为免费服务，但在迁移过程中会涉及到少量的阿里云ECS资源计费。详情请参见[计费概述](../../../../cn.zh-CN/产品定价/计费概述.md#)。
...
-   迁移数据目录范围说明
    ...
    -   Windows服务器：默认只迁移系统盘分区C盘（包括挂载在C盘下的其他目录），生成一个系统盘分区。如果您需要迁移其他分区如D盘，请参见[数据盘配置](../../../../cn.zh-CN/.md#table_b52_fiy_2vv)。
    -   Linux服务器：默认会迁移系统根目录下的所有目录（包括挂载在根目录/下的其他分区目录），生成一个系统盘分区。如果您需要迁移其他分区目录如/disk1，请参见[数据盘配置](../../../../cn.zh-CN/.md#table_b52_fiy_2vv)。
-   暂不支持迁移增量数据
