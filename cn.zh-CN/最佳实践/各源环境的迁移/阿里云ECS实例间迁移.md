...
阿里云ECS实例间迁移，建议您首先考虑使用阿里云的镜像复制和镜像共享功能。具体操作步骤，请参见[复制镜像](/cn.zh-CN/镜像/自定义镜像/复制镜像.md)和[共享或取消共享镜像](/cn.zh-CN/镜像/自定义镜像/共享或取消共享镜像.md) 。这两个功能不适用时，您可以参见下列场景完成准备工作并进行服务器迁移。

-   [相同VPC下的ECS实例迁移](#section_t7b_d0l_tvp)
-   [不同VPC下的ECS Windows实例迁移](#section_zbr_uxm_mii)
-   [不同VPC下的ECS Linux实例迁移](#section_qzr_wu9_bu0)

不同场景的准备工作完成后，即可开始正式的迁移操作。详情请参见[迁移服务器](#section_4vs_2dg_mk9)。如果您是第一次进行服务器迁移，建议先使用测试机演练。

## 相同VPC下的ECS实例迁移

此场景适用于ECS磁盘缩容。更多详情，请参见[云盘缩容](/cn.zh-CN/最佳实践/块存储/云盘缩容.md)。

在此场景中，建议在创建迁移任务时网络模式选择内网传输。使用内网传输能获得比通过公网更快速更稳定的数据传输效果，提高迁移工作效率。
...
请先了解注意事项，详情请参见[准备工作（迁移前必读）](/cn.zh-CN/用户指南/准备工作（迁移前必读）.md)。
