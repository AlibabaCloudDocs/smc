...
您可以参考本文档中的步骤，将华为云ECS实例迁移至阿里云。

在开始迁移服务器之前，您需要针对不同系统的虚拟机进行准备工作。详情请参见：

-   [华为ECS Windows系统迁移至阿里云](#section_jg5_rha_uof)
-   [华为ECS Linux系统迁移至阿里云](#section_jyy_9ds_g7f)

准备工作完成后，即可开始正式的迁移操作。详情请参见[迁移虚拟机至阿里云](#section_5al_6d6_f0e)。如果您是第一次进行服务器迁移，建议先使用测试机演练。
...
-   检查系统中是否安装了QEMU Guest Agent VSS Provider服务。
    ...
    如果已安装该服务，您可以尝试去类似于C:\\Program Files \(x86\)\\virtio\\monitor的目录下找到并执行uninstall.bat脚本，卸载QEMU Guest Agent软件。
...
-   请确保已安装Rsync库。安装方式请参见[如何安装Rsync](/cn.zh-CN/常见问题/SMC FAQ.md)。
-   检查授权应用。源服务器迁移到阿里云后，系统底层硬件设备会发生变化，可能会导致一些跟硬件绑定的应用许可证（license）失效，您需要做好检查。
