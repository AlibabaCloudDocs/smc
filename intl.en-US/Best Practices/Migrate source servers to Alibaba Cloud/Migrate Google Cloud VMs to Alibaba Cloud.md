...
-   Check the network environment.
    -   For information about how to perform migration across regions outside mainland China, see [Migrate Google Cloud VMs to Alibaba Cloud across regions outside mainland China](#section_thr_bib_rnq).
    -   If a VPC is available, we recommend that you perform migration over the VPC. Compared with migration over the Internet, migration over a VPC is more efficient and stable.
...
-   If your destination image is a container image, deploy applications by using the container image. For more information, see [Migrate source servers to Container Registry](/intl.en-US/Best Practices/Migrate source servers to Container Registry.mdsection_840_v5e_lea).
