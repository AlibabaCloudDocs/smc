...
阿里云ECS实例间迁移，建议您首先考虑使用阿里云的镜像复制和镜像共享功能。具体操作步骤，请参见[复制镜像](/intl.zh-CN/镜像/自定义镜像/复制镜像.md)和[共享或取消共享镜像](/intl.zh-CN/镜像/自定义镜像/共享或取消共享镜像.md) 。这两个功能不适用时，您可以参见下列场景完成准备工作并进行服务器迁移。
...
此场景适用于ECS磁盘缩容。更多详情，请参见[云盘缩容](/intl.zh-CN/最佳实践/块存储/云盘缩容.md)。
...
请先了解注意事项，详情请参见[准备工作（迁移前必读）](/intl.zh-CN/用户指南/准备工作（迁移前必读）.md)。
